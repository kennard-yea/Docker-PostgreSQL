...
mkdir -p /var/lib/pgadmin/storage/"$(printf ${PGADMIN_DEFAULT_EMAIL} | sed 's/@/_/g')"
[[ -f /tmp/.pgpass ]] && \
    mv /tmp/.pgpass /var/lib/pgadmin/storage/"$(printf ${PGADMIN_DEFAULT_EMAIL} | sed 's/@/_/g')"/ && \
    chmod 0600 /var/lib/pgadmin/storage/"$(printf ${PGADMIN_DEFAULT_EMAIL} | sed 's/@/_/g')"/.pgpass
